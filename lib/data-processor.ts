...
export function isClearEvent(event: GameEvent): boolean {
    return (
        event.eventAction === "clear" || event.eventAction === "clearIsFirst"
    );
}
...
export function filterEvents(
    ...
    if (options.selectedCountries.length > 0) {
        filtered = filtered.filter((event) => {
            ...
            return (
                event.clientIpCountryCode &&
                options.selectedCountries.includes(event.clientIpCountryCode)
            );
        });
    }
...
export function calculateStageStats(events: GameEvent[]): StageStats[] {
    ...
    stageMap.forEach((stageEvents, stageId) => {
        ...
        const clearRate =
            totalAttempts > 0 ? (clears / totalAttempts) * 100 : 0;
...
export function calculateFunnelData(events: GameEvent[]): FunnelData[] {
    ...
    for (let level = 1; level <= 20; level++) {
        ...
        const dropRate =
            reachedThisLevel > 0 ? (dropped / reachedThisLevel) * 100 : 0;
...
export function calculateStageAttrition(
    ...
    return sortedStats.map((stage, index) => {
        ...
        if (index > 0) {
            ...
            attritionRate =
                prevAttempts > 0 ? (attritionCount / prevAttempts) * 100 : 0;
        }
...
export function calculateUserStageStats(events: GameEvent[]): UserStageStats[] {
    ...
    events.forEach((event) => {
        ...
        if (isTryEvent(event)) {
            ...
            const currentCount =
                stageData.userTryAttempts.get(event.userId) || 0;
        ...
        if (isClearEvent(event) || isFailEvent(event)) {
            const currentCount =
                stageData.userClearFailCount.get(event.userId) || 0;
    ...
    Array.from(stageUserMap.entries())
        ...
        .forEach(([stageId, data]) => {
            ...
            const userClearRate =
                uniqueUsers > 0 ? (usersCleared / uniqueUsers) * 100 : 0;
            const clearProbability =
                totalClears + totalFails > 0
                    ? (totalClears / (totalClears + totalFails)) * 100
                    : 0;
...
/**
 * Parse revive events from CSV data
 * Expects CSV with stage_revive events containing stage_id, revive_type, revive_count
 */
export function parseReviveEvents(csvData: any[]): ReviveEvent[] {
    const reviveEvents: ReviveEvent[] = [];

    csvData.forEach((row) => {
        const eventAction = row["Event Action"];
        const eventCategory = row["Event Category"];

        // Process StageRevive events (Event Action: "StageRevive" or Event Category contains "stage_revive")
        if (
            eventAction === "StageRevive" ||
            eventAction === "stage_revive" ||
            eventCategory?.includes("stage_revive")
        ) {
            const customProps = row["Custom Event Properties"]
                ? JSON.parse(row["Custom Event Properties"])
                : {};

            // stage_id comes from Custom Event Properties (can be number or string)
            const stageId = customProps.stage_id?.toString() || "";

            if (stageId) {
                reviveEvents.push({
                    stageId,
                    reviveType: customProps.revive_type || "unknown",
                    reviveCount: parseInt(customProps.revive_count) || 1,
                    userId: row["User ID"] || undefined,
                });
            }
        }
...
/**
 * Calculate revive statistics per stage
...
export function calculateStageReviveStats(
    ...
    stageMap.forEach((events, stageId) => {
        ...
        events.forEach((event) => {
            // Track revive types
            const reviveType = event.reviveType;
        ...
        result.push({
            ...
            reviveCountDistribution,
            reviveTypeDistribution: reviveTypeCount,
            averageRevivePerGame,
        });
    });

    return result.sort((a, b) => a.stageId.localeCompare(b.stageId));
}

/**
 * Get overall revive statistics summary
 */
